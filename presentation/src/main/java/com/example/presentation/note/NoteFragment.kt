...
class NoteFragment : BaseFragment(R.layout.fragment_note) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setFragmentResultListener(RECORD_RESULT) { _, bundle ->
            val hasRecord = bundle.getBoolean(RECORD_HAS)
            viewModel.dispatch(NoteAction.HasRecordNoteChanged(hasRecord))
        }
    }
    ...
    override fun setupViews() {
        ...
        if (!viewModel.uiStateFlow.value.isFirstTime) {
            ...
            viewModel.dispatch(NoteAction.GetListRecordNote)
        }
        setupRecyclerView()
        setupClickListener()
        setupTextInput()
        ...
        viewModel.singleEventFlow.collectInViewLifecycle(this) { event ->
            when (event) {
                ...
                is NoteSingleEvent.GetListRecord -> {
                    ...
                    viewModel.dispatch(NoteAction.HasRecordNoteChanged(event.list.isNotEmpty()))
                }
    ...
    private fun collectStateUpdates() {
        viewModel.uiStateFlow.collectInViewLifecycle(this) { state ->
            binding.btnSaveNote.isVisible = !state.titleNote.isNullOrEmpty()
        }
        viewModel.loadingStateFlow.collectIn(viewLifecycleOwner) { state ->
    ...
    private fun initItemNote(noteModel: NoteModel) = binding.apply {
        ...
        viewModel.dispatch(
            NoteAction.SaveFileMediaToTemp(noteModel)
        )
        ...
        viewModel.dispatch(NoteAction.HasImageNoteChanged(noteModel.hasImage))
        viewModel.dispatch(NoteAction.HasRecordNoteChanged(noteModel.hasRecord))
    }
    ...
    private fun handleSaveClick() {
        viewModel.dispatch(NoteAction.DeleteDirectory)
        viewModel.dispatch(NoteAction.SaveNote(noteModel, actionNote))
    }
    ...
    companion object {
        ...
        const val IMAGE_HAS = "IMAGE_HAS"
        const val RECORD_RESULT = "RECORD_RESULT"
        const val RECORD_HAS = "RECORD_HAS"
    }
}